import { WebGPUAPI } from './lib/WebGPUAPI'
...
import vertexShaderString from './shaders/draw.vert.wgsl?raw'
import fragmentShaderString from './shaders/draw.frag.wgsl?raw'
import computeShaderString from './shaders/thermal.comp.wgsl?raw'
// import computeWallShaderString from './shaders/drawWall.comp.wgsl?raw'
...
export class Simulator {
  constructor(canvas) {
    ...
    document.onmousemove = (e) => {
      this.mousePos = [e.clientX / this.canvas.width, 1 - e.clientY / this.canvas.height]
      this.onMouseEvent()
    }
  ...
  async initialize() {
    // Initialize
    this.api = new WebGPUAPI(this.canvas)
    ...
    //   "HEAT EQUATION",
    //   "The <c>heat equation</c> describes the evolution of sources of heat in matter. This simulation uses periodic boundary conditions."
    // );
    // Physics config
    // let physicsFol = this.gui.addFolder("Physics");
    // this.conductivity = this.gui.addInput(
    //   "$$\\kappa$$",
    //   physicsFol,
    //   0.2,
    //   0,
    //   1,
    //   0.001,
    //   "The <c>thermal conductivity</c> describes how a material distribute heat inside it."
    // );
    this.conductivity = () => 0.2
    // this.density = this.gui.addInput(
    //   "$$\\rho$$",
    //   physicsFol,
    //   10,
    //   0,
    //   100,
    //   0.1,
    //   "The <c>density</c> of a material describes how particles are arranged inside the materials."
    // );
    this.density = () => 10
    // this.capacity = this.gui.addInput(
    //   "$$C$$",
    //   physicsFol,
    //   20,
    //   0,
    //   50,
    //   0.1,
    //   "The <c>thermal capacity</c> of a material describes how much the material can retain heat."
    // );
    this.capacity = () => 20

    // // Drawing mode
    // let drawFol = this.gui.addFolder("Add elements");
    // this.pencilMode = this.gui.addSelect(
    //   "Draw mode",
    //   drawFol,
    //   ["None", "Sources", "Walls"],
    //   "None",
    //   "Right click to <c>draw sources or walls</c> if selected"
    // );
    this.pencilMode = () => 'Sources'
    // this.wallDrawRadius = this.gui.addInput(
    //   "Wall Stencil Radius",
    //   drawFol,
    //   10,
    //   1,
    ...
    //   0.1
    // );
    // this.wallDrawRadius = () => 10
    // this.gui.addButton("Reset", drawFol, () => {
    //   this.sources = [0.0];
    //   this.api.updateBuffer(
    //     this.computeData.sources,
    //     new Float32Array(this.maxSourcesCount * 4)
    //   );
    //   this.api.updateBuffer(
    //     this.computeData.grid0,
    //     new Float32Array(this.canvas.width * this.canvas.height).fill(0)
    //   );
    //   this.api.updateBuffer(
    //     this.computeData.grid1,
    //     new Float32Array(this.canvas.width * this.canvas.height).fill(0)
    //   );
    // });
    ...
    this.computeData = {
      ...
      grid0: this.api.createBuffer(
        new Float32Array(this.canvas.width * this.canvas.height).fill(0),
        GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST
      ),
      grid1: this.api.createBuffer(
        new Float32Array(this.canvas.width * this.canvas.height).fill(0),
        GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST
      ),
      // Grid parameters
      ...
      // Physics parameters
      gridData: this.api.createBuffer(
        new Float32Array([this.canvas.width, this.canvas.height, deltat, deltax, deltay, 0, 0]),
        GPUBufferUsage.UNIFORM
      ),
      physData: this.api.createBuffer(
        new Float32Array([this.conductivity(), this.density(), this.capacity(), 0, 0, 0, 0]),
        GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST
      ),
      // Wall buffer
      //   wallData: this.api.createBuffer(
      //     new Uint32Array(this.maxWallsCount * 3 + 2),
      //     GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST
      //   ),
    }
    // this.newWalls = [];
    ...
    // Graphics pipeline
    this.graphicsPip0 = await this.api.createGraphicsPipeline({
      // Draw 0
      shaders: [
        // { type: "vert", src: "shaders/draw.vert.wgsl" },
        // { type: "frag", src: "shaders/draw.frag.wgsl" },
        { type: 'vert', src: vertexShaderString },
        { type: 'frag', src: fragmentShaderString },
      ],
      layouts: [
        [
          {
    ...
    this.graphicsPip1 = await this.api.createGraphicsPipeline({
      // Draw 1
      shaders: [
        // { type: "vert", src: "shaders/draw.vert.wgsl" },
        // { type: "frag", src: "shaders/draw.frag.wgsl" },
    ...
    // Compute pipeline
    this.computePip0 = await this.api.createComputePipeline({
      // Read from 1, write to 0
      shader: {
        type: 'comp',
        // src: "shaders/thermal.comp.wgsl",
        src: computeShaderString,
      },
      layouts: [
        [
          {
    ...
    this.computePip1 = await this.api.createComputePipeline({
  ...
  async tick(dt) {
    ...
    // Draw Walls
    // if (
    //   this.newWalls.length != 0 &&
    //   this.drawCounter >= this.drawWallTickCount
    // ) {
    //   // Set walls buffer
    ...
    //     newData = [
    //       this.newWalls[this.newWalls.length - 3],
    //       this.newWalls[this.newWalls.length - 2],
    //       this.newWalls[this.newWalls.length - 1],
    //     ];
    //   this.newWalls = [];
    //   this.drawCounter = 0;
    // }
